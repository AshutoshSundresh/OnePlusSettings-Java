...
public final class CollapsingTextHelper {
    ...
    private void calculateBaseOffsets() {
        ...
        if (i == 48) {
            ...
            float descent2 = descent - this.textPaint.descent();
            if (shouldDrawMultiline()) {
                f2 = ((float) this.collapsedBounds.centerY()) - descent;
            } else {
                f2 = descent2 + ((float) this.collapsedBounds.centerY());
            }
            this.collapsedDrawY = f2;
        } else {
        ...
        if (i3 == 48) {
            ...
            float descent3 = ((this.textPaint.descent() - this.textPaint.ascent()) / 2.0f) - this.textPaint.descent();
            if (shouldDrawMultiline()) {
                f = ((float) this.expandedBounds.centerY()) - (height / 2.0f);
            } else {
                f = descent3 + ((float) this.expandedBounds.centerY());
            }
            this.expandedDrawY = f;
        } else {
            if (shouldDrawMultiline()) {
                f4 = height - this.textPaint.descent();
            }
            this.expandedDrawY = ((float) this.expandedBounds.bottom) - f4;
        }
    ...
    public void draw(Canvas canvas) {
        ...
        if (this.textToDraw != null && this.drawTitle) {
            ...
            float lineAscent = (float) this.textLayout.getLineAscent(0);
            float f3 = this.scale;
            if (f3 != 1.0f) {
                canvas.scale(f3, f3, f, f2);
            }
            if (z) {
                canvas.drawBitmap(this.expandedTitleTexture, f, f2 + lineAscent, this.texturePaint);
                canvas.restoreToCount(save);
                return;
            }
            if (shouldDrawMultiline()) {
                drawMultinlineTransition(canvas, lineLeft, f, f2, lineAscent);
            } else {
    ...
    private void drawMultinlineTransition(Canvas canvas, float f, float f2, float f3, float f4) {
        ...
        canvas.drawText(trim, 0, Math.min(this.textLayout.getLineEnd(0), trim.length()), 0.0f, f6 / this.scale, (Paint) this.textPaint);
    }
