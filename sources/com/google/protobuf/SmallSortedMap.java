...
public class SmallSortedMap<K extends Comparable<K>, V> extends AbstractMap<K, V> {
                ...
                if (!isImmutable()) {
                    for (int i = 0; i < getNumArrayEntries(); i++) {
                        ...
                        if (((FieldSet.FieldDescriptorLite) arrayEntryAt.getKey()).isRepeated()) {
                            arrayEntryAt.setValue((V) Collections.unmodifiableList(arrayEntryAt.getValue()));
                        }
                    }
                    for (Map.Entry<K, V> entry : getOverflowEntries()) {
    ...
    public V get(Object obj) {
        ...
        return binarySearchInArray >= 0 ? (V) this.entryList.get(binarySearchInArray).getValue() : this.overflowEntries.get(comparable);
    }
    ...
    public V put(K k, V v) {
        ...
        if (binarySearchInArray >= 0) {
            return (V) this.entryList.get(binarySearchInArray).setValue(v);
        }
        ...
        if (size == i2) {
            ...
            getOverflowEntriesMutable().put((K) remove.getKey(), (V) remove.getValue());
        }
    ...
    private V removeArrayEntryAt(int i) {
        checkMutable();
        V v = (V) this.entryList.remove(i).getValue();
        if (!this.overflowEntries.isEmpty()) {
    ...
    public class Entry implements Map.Entry<K, V>, Comparable<SmallSortedMap<K, V>.Entry> {
        ...
        /* JADX WARN: Type inference failed for: r0v1, types: [K, K extends java.lang.Comparable<K>] */
    ...
    private class EntryIterator implements Iterator<Map.Entry<K, V>> {
        ...
        /* JADX DEBUG: Type inference failed for r1v1. Raw type applied. Possible types: java.util.Iterator<java.util.Map$Entry<K, V>>, java.util.Iterator<java.util.Map$Entry<K extends java.lang.Comparable<K>, V>> */
