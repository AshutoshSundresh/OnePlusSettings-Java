...
public class SwitchCompat extends CompoundButton {
    ...
    public SwitchCompat(Context context, AttributeSet attributeSet, int i) {
        ...
        if (Build.VERSION.SDK_INT >= 29) {
            saveAttributeDataForStyleable(context, R$styleable.SwitchCompat, attributeSet, obtainStyledAttributes.getWrappedTypeArray(), i, 0);
        }
        ...
        setRadius(obtainStyledAttributes.getDimensionPixelSize(R$styleable.SwitchCompat_android_radius, -1));
    ...
    public void setVibrateSceneId(int i) {
        this.mVibratorSceneId = i;
    }

    public boolean performClick() {
        if (VibratorSceneUtils.systemVibrateEnabled(getContext())) {
            long[] vibratorScenePattern = VibratorSceneUtils.getVibratorScenePattern(getContext(), this.mVibrator, this.mVibratorSceneId);
    ...
    private void setRadius(int i) {
        if (i != -1) {
            ...
            if (background == null || !(background instanceof RippleDrawable)) {
                Log.i("SwitchCompat", "setRaidus fail , background not a rippleDrawable");
                return;
            }
            background.mutate();
